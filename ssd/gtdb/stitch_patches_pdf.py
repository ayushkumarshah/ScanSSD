...
sys.path.extend(["/home/as1211/Desktop/MathSeer/ScanSSD/ssd"])
import cv2
import os
import numpy as np
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Stitching method")

    parser.add_argument("--data_file", default="test", type=str, help="choose one")
    parser.add_argument("--output_dir", default=".", help="Output directory path")
    parser.add_argument("--math_dir", required=True, type=str, help="detections dir")
    parser.add_argument(
        "--math_ext", default=".csv", help="Extention of detection files"
    )
    parser.add_argument(
        "--home_data", default="/home/psm2208/data/GTDB/", type=str, help="database dir"
    )
    parser.add_argument(
        "--home_eval", default="/home/psm2208/code/eval/", type=str, help="Eval dir"
    )
    parser.add_argument(
        "--home_images",
        default="/home/psm2208/data/GTDB/images/",
        type=str,
        help="Images dir",
    )
    parser.add_argument(
        "--home_anno",
        default="/home/psm2208/data/GTDB/annotations/",
        type=str,
        help="Annotations dir",
    )
    parser.add_argument(
        "--home_char",
        default="/home/psm2208/data/GTDB/char_annotations/",
        type=str,
        help="Char anno dir",
    )
    parser.add_argument(
        "--stitching_algo", default="equal", type=str, help="Stitching algo to use"
    )
    parser.add_argument(
        "--algo_threshold", default=30, type=int, help="Stitching algo threshold"
    )
    parser.add_argument("--num_workers", default=4, type=int, help="Number of workers")
    parser.add_argument(
        "--preprocess", type=bool, help="Whether to fit math regions before pooling"
    )
    parser.add_argument(
...
def voting_algo(params):
    ...
    image = cv2.imread(
        os.path.join(args.home_images, pdf_name, str(int(page_num + 1)) + ".png")
    )
...
def stitch(args):
    ...
    for pdf_name in pdf_list:
        ...
        for page_num in pages:
            current_math = math_regions[pdf_name][
                math_regions[pdf_name][:, 0] == page_num
            ]
            voting_ip_list.append(
                [args, np.delete(current_math, 0, 1), pdf_name, page_num]
            )
