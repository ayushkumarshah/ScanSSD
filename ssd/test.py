...
"""
# uncomment this to hide the package deprecation warning that may pop up depending on your version of torch
import sys
...
def test_net_batch(args, net, gpu_id, dataset, transform, thresh):
    ...
    for batch_idx, (images, targets, metadata) in enumerate(data_loader):
        ...
        for img, meta in zip(images, metadata):
            ...
            while (
                j < detections.size(2) and detections[k, i, j, 0] >= thresh
            ):  # TODO it was 0.6
...
def test_gtdb(args):
    ...
    # initialize SSD
    net = build_ssd(
        args, "test", exp_cfg[args.cfg], gpu_id, args.model_type, num_classes
    )
    ...
    """
    # NOTE: if things aren't working, you may need to replace the following two lines with these.
    """
    ...
    # net.load_state_dict(
    #     torch.load(args.trained_model, map_location={"cuda:1": "cuda:0"})
    # )
